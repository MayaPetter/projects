...
static Object_t *new(Metadata_t *metadata);
...
Metadata_t animal_meta = {"animal", sizeof(Animal_t),
                          &obj_meta, &animal_methods};
Metadata_t dog_meta = {"dog", sizeof(Dog_t), &animal_meta, &dog_methods};
Metadata_t cat_meta = {"cat", sizeof(Cat_t), &animal_meta, &cat_methods};
Metadata_t legendary_meta = {"legendary", sizeof(Legen_t),
                              &cat_meta, &legendary_methods};
...
void AnimalInitFileds(Animal_t *this, int num_masters)
{
	this->ID = ++G_COUNTER;
	this->num_legs = 5;
...
{
    AnimalInitFileds(this, 1);
    puts("Instance initialization block Animal");
	puts("Animal Cotr");
	(*((Metadata_t *)this)->method)[SAY_HELLO](this);
	(*((Metadata_t *)this)->method)[SHOW_COUNTER](this);
	
	printf("%s\n", (char *)(*((Metadata_t *)this)->method)[TO_STRING](this));
	printf("%s\n", (char *)(*((Metadata_t *)this)->method)[TO_STRING](this));	
}

static void AnimalCotrInt(Animal_t *this, int num_masters)
{
    AnimalInitFileds(this, num_masters);
...
int main()
{
	...
	(*((Metadata_t *)animal)->method)[SHOW_COUNTER](animal);
	...
	/** Animal array - new animals **/
	{
		...
		for (i = 0; 5 > i; ++i)
		{
			(*((Metadata_t *)animal_arr[i])->method)[SAY_HELLO](animal_arr[i]);
			printf("%d\n", (*((Metadata_t *)animal_arr[i])->method)[GET_MASTERS](animal_arr[i]));
		}
		
		for (i = 0; 5 > i; ++i)
		{
			java2Cfoo(animal_arr[i]);
		}
		
		for (i = 4; 0 <= i; --i)
		{
			(*((Metadata_t *)animal_arr[i])->method)[FINALIZE](animal_arr[i]);
		}
	}
		
		(*((Metadata_t *)la)->method)[FINALIZE](la);
		(*((Metadata_t *)cat)->method)[FINALIZE](cat);
		(*((Metadata_t *)dog)->method)[FINALIZE](dog);
		(*((Metadata_t *)animal)->method)[FINALIZE](animal);
